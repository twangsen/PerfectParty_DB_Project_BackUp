...
mod = Blueprint('supplies', __name__)

col = sql_helper.column_name["supply"]
...
@mod.route('/supply/select', methods=['GET', 'POST'])
def get_supply():
    if request.method == 'POST':
        ...
        cur = con.cursor()
        if not where_args:
            cur.execute('SELECT {} FROM supply {}{}'.format(select_clause, group_clause, have_clause))
        else:
            cur.execute('SELECT {} FROM supply WHERE {}{}{}'.format(select_clause, where_clause, group_clause, have_clause))
        output = cur.fetchone()
        results = []
...
@mod.route('/supply/update', methods=['PUT'])
def update_supply():
    if not request.is_json:
        message = "Request is not json format"
        return make_response(jsonify(message), 400)
    ...
    cur = con.cursor()
    try:
        cur.execute('Update supply SET {} WHERE {}'.format(set_clause, where_clause))
...
@mod.route('/supply/insert', methods=['PUT'])
def insert_supply():
    if not request.is_json:
        message = "Request is not json format"
        return make_response(jsonify(message), 400)
    ...
    cur = con.cursor()
    try:
        cur.execute('INSERT INTO supply {} Values {}'.format(into_clause, values_clause))
...
@mod.route('/supply/delete', methods=['DELETE'])
def delete_supply():
    if not request.is_json:
        message = "Request is not json format"
        return make_response(jsonify(message), 400)
    ...
    cur = con.cursor()
    try:
        cur.execute('Delete from supply WHERE {}'.format(where_clause))
