...
col = sql_helper.column_name["client"]

@mod.route('/client')
def index():
    return "index"

@mod.route('/client/select', methods=['GET', 'POST'])
def get_client():
    if request.method == 'POST':
        ...
        cur = con.cursor()
        if not where_args:
            cur.execute('SELECT {} FROM Client{}{}'.format(select_clause, group_clause, have_clause))
        else:
            cur.execute('SELECT {} FROM Client WHERE {}{}{}'.format(select_clause, where_clause, group_clause, have_clause))
        output = cur.fetchone()
        results = []
...
@mod.route('/client/update', methods=['PUT'])
def update_client():
    if not request.is_json:
        message = "Request is not json format"
        return make_response(jsonify(message), 400)
    ...
    cur = con.cursor()
    try:
        cur.execute('Update Client SET {} WHERE {}'.format(set_clause, where_clause))
...
@mod.route('/client/insert', methods=['PUT'])
def insert_client():
    if not request.is_json:
        message = "Request is not json format"
        return make_response(jsonify(message), 400)
    ...
    cur = con.cursor()
    try:
        cur.execute('INSERT INTO Client {} Values {}'.format(into_clause, values_clause))
...
@mod.route('/client/delete', methods=['DELETE'])
def delete_client():
    if not request.is_json:
        message = "Request is not json format"
        return make_response(jsonify(message), 400)
    ...
    cur = con.cursor()
    try:
        cur.execute('Delete from Client WHERE {}'.format(where_clause))
...
@mod.route('/client/id', methods=['GET'])
def get_nextid():
    # connect to DB
    con = db.get_connection()
    cur = con.cursor()
    cur.execute('Select last_value + 1 FROM client_clientid_seq')
    output = cur.fetchone()
